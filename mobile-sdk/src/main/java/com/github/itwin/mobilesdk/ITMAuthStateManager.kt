 ...
 *
 * @param itmApplication The [ITMApplication] used to provide a logger and application context.
 */
class ITMAuthStateManager private constructor(private val itmApplication: ITMApplication) {
    private val context = itmApplication.appContext
    ...
    companion object {
        ...
        private val INSTANCE_REF: AtomicReference<WeakReference<ITMAuthStateManager>> =
            AtomicReference(WeakReference(null))
        private const val STORE_NAME = "ITMAuthState"
         ...
         */
        @AnyThread
        fun getInstance(itmApplication: ITMApplication): ITMAuthStateManager {
            var manager: ITMAuthStateManager? = INSTANCE_REF.get().get()
            if (manager == null) {
                manager = ITMAuthStateManager(itmApplication)
